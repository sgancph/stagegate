...
From the repository root, `./dev` is the canonical one-command startup. It checks whether Ollama is reachable, then runs the Next.js development server for both the frontend and API routes. Next.js watches `app/` and updates the local page automatically on save. The basic-auth gate is skipped in development, so the local app is open. Local development does not update Vercel.
