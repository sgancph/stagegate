...
const subs: { name: string; gate: string; chip: string; dot: string; chipMod?: string }[] = [
  ...
  { name: 'Public Golf Apartments Phase 2', gate: 'Stage Gate 2', chip: 'Incomplete', dot: 'red', chipMod: 'red' },
];

const recs = [
  { title: 'Review & send outcome letter', sub: 'Music Theme Park SG3 — review outcome synced from Teams this morning.', mod: 'blue' },
  { title: 'Return Public Golf Apartments SG2 to team', sub: 'Completeness scan failed — 2 mandatory documents missing.', mod: 'amber' },
];
    ...
    <div className="sec-dash">
      ...
      <section className="banner">
        <div className="banner__left">
          <span className="banner__icon"><Icon name="bell" /></span>
          <div className="banner__content">
            <p className="banner__title">New submission: <strong>Arena · Stage Gate 3</strong> just arrived in your intake queue.</p>
          </div>
        </div>
        <div className="banner__right">
          <span className="banner__meta">Received 08:30</span>
          <button className="btn btn--white" onClick={() => navigate('reports')}>Review submission<Icon name="arrowRight" size={15} strokeWidth={2} /></button>
        </div>
      </section>
      ...
      <div className="sec-kpis">
        {kpis.map((k) => (
          <div key={k.label} className="sec-kpi">
            ...
            <div className="sec-kpi__stat"><span className={`ws-tag ws-tag--${k.variant}`}>{k.tag}</span></div>
          </div>
        ))}
      </div>
      ...
      <div className="sec-grid">
        <section className="card">
          <div className="card__head"><div><h2 className="card__title">Pipeline overview</h2></div></div>
          <div className="sec-funnel">
            {funnel.map((s) => (
              <div key={s.label} className={`sec-stage sec-stage--${s.mod}`}>
                <span className="sec-stage__label">{s.label}</span>
                <div className="sec-stage__box"><span className="sec-stage__num">{s.num}</span></div>
              </div>
            ))}
          </div>
          ...
          <div className="sec-tabs">
            ...
            <button className="ws-tab is-active">My queue<span className="sec-tab-count">7</span></button>
            <button className="ws-tab">Overdue<span className="sec-tab-count sec-tab-count--alert">3</span></button>
          </div>
          <div className="sec-subs">
            {subs.map((s) => (
              <div key={s.name} className="sec-sub" role="button" tabIndex={0} onClick={() => navigate('reports')}>
                <span className="dot" style={{ background: ({ blue: '#0BC0FF', amber: 'var(--amber)', red: 'var(--red)' } as Record<string, string>)[s.dot] }} />
                <div className="sec-sub__text"><p className="sec-sub__name">{s.name}</p><p className="sec-sub__sub">{s.gate}</p></div>
                <div className="sec-sub__act"><span className={`sec-chip${s.chipMod ? ` sec-chip--${s.chipMod}` : ''}`}>{s.chip}</span></div>
              </div>
            ))}
          </div>
        </section>
        ...
        <aside className="sec-side">
          <section className="card">
            ...
            <div className="sec-recs">
              {recs.map((r) => (
                <div key={r.title} className="sec-rec" role="button" tabIndex={0} onClick={() => navigate('reports')}>
                  <span className={`sec-rec__ic sec-rec__ic--${r.mod}`}><Icon name={r.mod === 'blue' ? 'reports' : 'bell'} size={16} /></span>
                  <div className="sec-rec__body"><p className="sec-rec__title">{r.title}</p><p className="sec-rec__sub">{r.sub}</p></div>
                  <span className="sec-rec__arrow"><Icon name="chevronRight" size={16} strokeWidth={2} /></span>
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="sec-panels">
              {panels.map((p) => (
                <div key={p.name} className="sec-panel">
                  <div><p className="sec-panel__name">{p.name}</p><p className="sec-panel__date">{p.date}</p></div>
