...
const SECTIONS = [
  '1 · Strategic Case', '2 · Scope & Design', '3 · Commercial & Financial Case',
  '4 · Delivery & Procurement', '5 · Risk & Assurance', '6 · Operational Readiness',
];
const SECTION_BODY: Record<number, JSX.Element> = {
  2: <p className="ws-prose">Detailed design is complete across all disciplines. The exterior concept has been value-engineered against the approved <strong>Cost Plan</strong>, with a 6% saving reallocated to operational readiness scope.</p>,
  3: <p className="ws-prose">The Arena delivers a projected <strong>IRR of 12.4%</strong> against a total development cost of <strong>SAR 1.84bn</strong>. Phase 1 contingency is set at 8% of construction value, consistent with the approved Cost Plan. Revenue is underpinned by a blended events calendar and anchor-tenant commitments covering 64% of leasable area.</p>,
  4: <p className="ws-prose">Procurement moves to a two-stage design-and-build with early contractor involvement. The recommended main works contractor shortlist is evidenced in the <strong>Consultant Procurement Strategy</strong>.</p>,
};
...
export function AuthoringWorkspace() {
  ...
  const onPick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles((f) => [...picked.map((p) => ({ name: p.name, meta: `${(p.size / 1048576).toFixed(1)} MB` })), ...f]);
      ...
      <div className="ws-grid">
        ...
        <aside className="ws-left">
          <section className="ws-panel">
            ...
            <input ref={fileInput} type="file" multiple hidden onChange={onPick} accept=".pdf,.doc,.docx,.xls,.xlsx" />
            <button type="button" className="ws-dropzone" style={{ width: '100%', cursor: 'pointer' }} onClick={() => fileInput.current?.click()}>
            ...
            <div className="ws-files">
              <div className="ws-restructure">
                <div className="ws-restructure__top">
                  ...
                  <button className="ws-file-del" aria-label="Remove deliverable" onClick={() => toast('Project Execution Plan removed')}><Icon name="x" size={10} strokeWidth={2.6} /></button>
                </div>
                <p className="ws-restructure__note">Uploaded in a non-standard layout — restructure in Word before drafting.</p>
                <button className="ws-restructure__btn" onClick={() => toast('Opening in the Word add-in…')}><Icon name="external" size={14} strokeWidth={1.9} />Open in Word</button>
              </div>
              {files.map((f) => (
                <div key={f.name} className="ws-file">
                  ...
                  <button className="ws-file-del" aria-label={`Remove ${f.name}`} onClick={() => setFiles((list) => list.filter((x) => x.name !== f.name))}><Icon name="x" size={10} strokeWidth={2.6} /></button>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="ws-mid">
          <section className="ws-panel">
            <div className="ws-draft-head">
              <div><p className="ws-draft-title">Arena SG3 — Stage Gate Report</p><p className="ws-draft-sub">Generated from {files.length} source deliverables · SGRP Template v2.1</p></div>
              <div className="ws-draft-meta"><span className="ws-draft-meta__count">{reviewed ? '17' : '16'} / 19 sections reviewed</span></div>
            </div>
            ...
            <div className="ws-secnav">
              ...
              <div className="ws-steps">
                {SECTIONS.map((label, idx) => (
                  <button key={label} className={`ws-step ws-step--${stepMod(idx)}`} title={label} aria-label={label}
                    aria-current={idx === section ? 'true' : undefined} onClick={() => setSection(idx)}>{idx + 1}</button>
                ))}
              </div>
            </div>
            ...
            <div className="ws-sec-head">
              ...
              <div className="ws-sec-meta"><span className="ws-pill-soft">{reviewed && section === 2 ? 'Reviewed' : 'Drafted'}</span></div>
            </div>
            {SECTION_BODY[section] ?? <p className="ws-prose">This section has been drafted from the source deliverables and is ready for your review.</p>}

            <div className="ws-actbar">
              <div className="ws-tools">
                <button className="ws-tool" aria-label="Regenerate section" title="Regenerate" onClick={() => toast('Regenerating section…')}><Sparkle size={16} /></button>
                <button className="ws-tool" aria-label="Copy section" title="Copy" onClick={() => toast('Section copied')}><Icon name="file" size={16} /></button>
              </div>
              <div className="ws-actbar-right">
                {reviewed && section === 2 && <span className="ws-reviewed"><Icon name="shield" size={14} /> Reviewed by you</span>}
                <button className={`btn btn--sm ${reviewed && section === 2 ? 'btn--green' : 'btn--navy'}`}
                  onClick={() => { setReviewed(true); setSection(2); toast('Section marked as reviewed'); }}>
                  {reviewed && section === 2 ? 'Reviewed ✓' : 'Acknowledge review'}
                </button>
              </div>
            </div>
          </section>
        ...
        <aside className="ws-right">
          <section className="ws-panel">
            ...
            <div className="ws-att"><span className="ws-att__n">2</span> sections with open gaps<span className="ws-att__warn"><Icon name="alert" size={15} /></span></div>
          </section>
        </aside>
      </div>
    </div>
