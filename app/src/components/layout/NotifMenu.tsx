...
const NOTES: Record<'project' | 'secretariat', { items: Note[]; go: View; footer: string }> = {
  project: {
    go: 'reports', footer: 'View all in My reports',
    items: [
  ...
  secretariat: {
    go: 'reports', footer: 'View all submissions',
    items: [
...
export function NotifMenu() {
  ...
  useEffect(() => { setCleared(false); }, [persona]);

  const unread = persona === 'secretariat' ? 3 : 2;
  return (
    <div className="notif" ref={wrap}>
      <button className="icon-btn" aria-label="Notifications" aria-haspopup="true" aria-expanded={open}
      ...
      {open && (
        <div className="notif__pop" role="menu">
          <div className="notif__head"><span className="notif__title">Notifications</span>
            <button className="notif__clear" type="button" onClick={() => setCleared(true)}>Mark all read</button></div>
          <div className="notif__list">
            {cfg.items.map((n) => (
              <button key={n.t} className={`notif__item${n.read || cleared ? ' is-read' : ''}`} role="menuitem"
                onClick={() => { setOpen(false); navigate(cfg.go); }}>
                <span className="notif__dot" />
                <span className="notif__body"><p className="notif__t">{n.t}</p><p className="notif__meta">{n.meta}</p></span>
              </button>
            ))}
          </div>
          <div className="notif__foot"><button className="link" onClick={() => { setOpen(false); navigate(cfg.go); }}>{cfg.footer}</button></div>
        </div>
      )}
    </div>
  );
